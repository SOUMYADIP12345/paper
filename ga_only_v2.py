...
def plot_topology_montage(cfg: dict, max_tiles: int = 12,
                          out_path: str = "ga_topology_montage.png") -> None:
    """
    Combine all per-round topology snapshots into ONE grid image.

    Reads every `topology_snapshots/ga_round_XXXX.png` produced during the run,
    sorts them by round number, evenly samples up to `max_tiles` (so the figure
    stays readable when there are 50+ snapshots), and lays them out in a grid.
    Each tile is labelled with its round number so you can see the network
    evolve over time in a SINGLE output file - useful for putting one figure
    into the paper instead of many separate PNGs.

    Gracefully skipped when no snapshots exist (SNAPSHOT_EVERY was 0 or no
    round was a multiple of it).
    """
    ...
    for fname in os.listdir(SNAPSHOT_DIR):
        if not fname.startswith("ga_round_") or not fname.endswith(".png"):
            continue
        try:
            rnd = int(fname[len("ga_round_"):-len(".png")])
        except ValueError:
            continue
        entries.append((rnd, os.path.join(SNAPSHOT_DIR, fname)))
    if not entries:
        print(f"  !  No ga_round_*.png snapshots in {SNAPSHOT_DIR}/ - "
              f"montage skipped. Set SNAPSHOT_EVERY > 0 to generate them.")
        return
    ...
    for i, (rnd, path) in enumerate(entries):
    ...
    plt.close(fig)
    print(f"  Plot saved -> {out_path}   ({n} snapshots combined)")
