...
def run_protocol(cfg: dict, protocol: str):
    """Run one protocol end-to-end. Nodes are re-created with the SAME fixed
    seed for every protocol (create_nodes seeds 42), so all protocols see an
    identical field, batteries and per-node solar efficiencies. Returns
    (stats, first_dead, network_dead)."""
...
# ==============================================================================
# MAIN ENTRY POINT  (single file: run the full modern-baseline comparison)
# ==============================================================================

if __name__ == "__main__":
    # Default: run the full comparison (GA vs PSO vs GWO vs HEED vs LEACH) with
    # the default configuration - no prompts, ready for Colab / CI.
    #
    #   python solar_ga_wsn_with_baselines.py
    #   SOLAR_GA_PROTOCOLS="GA,PSO,GWO" python solar_ga_wsn_with_baselines.py
    #
    # To instead run only the original GA-vs-LEACH driver (with prompts / plots),
    # call run_ga_vs_leach() from your own script or a notebook.
    run_comparison()
